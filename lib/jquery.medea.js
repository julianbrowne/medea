    ...
    **/
    ...
    function medeaStyle() { 
        return $("<style>")
            ...
            .append("div.medea-form-buttons-row { padding: 0px 50px; }")
    ...
    function addEventHandlers(container) { 
        ...
        container.on(events.MEDEA_FIELD_ADD, function(e) { 
            var lastRow = elements.form.find("> div.medea-form-data-row").last();
            // there's no last row in an empty form
            if(lastRow.length === 0) { 
                lastRow = elements.form.find("div.medea-form-buttons-row");
