...
window.Medea = (function($) { 
    ...
    function buttons(container) { 
        ...
        var cancelButton = button("default").html("cancel").attr("id", "medea-cancel-btn");
        ...
        var addButton = button("success").html("add").attr("id", "medea-add-btn");
    ...
    function cloneButton(name, value, type) { 

        var name = name ? name : "name";
        var value = value ? value : "";
        var type = type ? type : "text";

        var btn = icon("duplicate", function() { 
            $(this)
                .closest("div.form-group")
                .after(dynamicInputElement(name, value, type));
        }).addClass("text-danger");

        if(type === "checkbox") { 
            btn.prop("checked", value);
        }
    ...
    **/
    ...
    function editableLabelFieldGroup(name) { 
        ...
        function saveLabelAndExit(e) { 
            ...
            var fullLabelPath = editor.val();
            var niceLabelText = regularCase(fullLabelPath.split(/\./).pop());
            label.html(niceLabelText);
            label.attr("for", fullLabelPath);
            editor.addClass("hidden");
            label.removeClass("hidden");
            var inputField = $("input").eq($("input").index(this) + 1);
            inputField.attr("name", fullLabelPath);
    ...
    function inputGroup(name, value, type) { 
        ...
        inputGroupHTML.append(inputGroupAddon(cloneButton(name, value, type)));
    ...
    function dynamicInputElement(name, value, type) { 
        ...
        formGroup.append(editableLabelFieldGroup(name));
        var inputValueWrapper = $("<div>").addClass("col-sm-" + options.inputColumns);
        var inputTextBox = inputGroup(name, value, type);
    ...
    function addEventHandlers(container) { 
        ...
        container.on(events.MEDEA_FIELD_ADD, function(e) { 
            //console.log(e);
            if(options.buttons) { 
                elements.form.find("div.form-group:last").before(dynamicInputElement());
            }
            else { 
                elements.form.append(dynamicInputElement());
            }
        });
    ...
    function modalise(selector) { 
        ...
        $("body").append( 
            '<div id="' + options.modalId + '" class="modal" role="dialog"> \
    ...
    $.fn.medea = function(object, opts) { 
        ...
        return this.each(function(index, element) { 
            ...
            if(options.modal) { modalise(container); }
